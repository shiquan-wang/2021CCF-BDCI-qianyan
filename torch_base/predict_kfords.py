...
num_epochs = 11  # 训练最大迭代次数
batch_size = 1000  # 训练时每个batch中的样本数 k-folds = 10
patience = 5  # 早停轮数
file_name = 'predict_k_folds_10'  # 指定输出文件的名字
k_folds_model_name = 'k_folds_10'  # 指定输出文件的名字
model_name_or_path = '/Work20/2020/wangshiquan/model/ernie_grame/'  # 预训练模型权重载入路径
train_input = '../data/train.txt'  # 完成预处理的训练集载入路径
#test_input = '../data/dev.txt'  # 完成预处理的测试集集载入路径
test_input = '../data/test_A.tsv'  # 完成预处理的测试集集载入路径
random_seed = 42  # 随机种子
...
fh = logging.FileHandler(file_path + file_name + 'log.txt')
...
def read_data(file_name, tta = False):
    examples = []
    with open(file_name, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            line = line.strip()
            if line and tta == False:
                line = line.split('\t')
                examples.append(InputExample(s1=line[0], s2=line[1], label=int(line[2]) if len(line) == 3 else None))
            elif line and tta == True:
                line = line.split('\t')
                examples.append(InputExample(s1 = line[1], s2=line[0], label=int(line[2]) if len(line) == 3 else None))
    return examples
...
#tta, 测试的时候翻转A B句的顺序，取平均，或者更可以取不同权重
test_examples_tta = read_data(test_input, tta=True)
test_features_tta = convert_examples_to_features(test_examples_tta, tokenizer, max_seq_length, True)
test_input_ids_tta = torch.tensor(select_field(test_features_tta, 'input_ids'), dtype=torch.long)
test_input_mask_tta = torch.tensor(select_field(test_features_tta, 'input_mask'), dtype=torch.long)
test_segment_ids_tta = torch.tensor(select_field(test_features_tta, 'segment_ids'), dtype=torch.long)
...
#for fold, (train_index, valid_index) in enumerate(skf.split(all_label, all_label)):#K-Folds 交叉验证
for fold in range(7):#K-Folds 交叉验证
    ...
    test_tta = torch.utils.data.TensorDataset(test_input_ids_tta, test_input_mask_tta, test_segment_ids_tta)
    test_loader_tta = torch.utils.data.DataLoader(test_tta, batch_size = batch_size, shuffle = False)
    ...
    #         if not os.path.exists('./model_save/' + file_name + "/"):
    #             os.mkdir('./model_save/' + file_name + "/")
    #         torch.save(model.state_dict(), './model_save/' + file_name + '/best_{}.bin'.format(fold))
    #     else:
    #         early_stop += 1
    #     logger.info(
    ...
    # 得到一折模型对测试集的预测结果
    model.load_state_dict(torch.load('./model_save/' + k_folds_model_name + '/best_{}.bin'.format(fold)))
    test_preds_fold = np.zeros((len(test_examples), 2))
    model.eval()
    print(f"test_loader_len : {len(test_loader)}")
    print(f"test_loader_tta_len : {len(test_loader_tta)}")
    with torch.no_grad():
        ...
        for batch_i, batch_tta_i in tqdm(zip(enumerate(test_loader), enumerate(test_loader_tta))):
            i = batch_i[0]
            batch = batch_i[1]
            batch_tta = batch_tta_i[1]
            batch = tuple(t.cuda() for t in batch)
            #tta
            batch_tta = tuple(t.cuda() for t in batch_tta)

            x_ids, x_mask, x_sids = batch
            #tta
            x_ids_tta, x_mask_tta, x_sids_tta = batch_tta
            with autocast():
                y_pred = model(x_ids, x_mask, x_sids).detach()
                #print(y_pred.shape)
                y_pred_tta = model(x_ids_tta, x_mask_tta, x_sids_tta).detach()
                #print(y_pred_tta.shape)
                y = (y_pred + y_pred_tta) / 2
            test_preds_fold[i * batch_size:(i + 1) * batch_size] = F.softmax(y, dim=1).cpu().numpy()
    
    # oof_train[valid_index] = valid_best
    # acc, f1 = metric(all_label[valid_index], np.argmax(valid_best, axis=1))
    # logger.info('epoch: best, acc: %.8f, f1: %.8f, best_acc: %.8f\n' %
    #             (acc, f1, best_acc))
...
# np.savetxt(submit_path + file_name + '/train_submit.txt', oof_train)
...
logger.info(f"y_preds nums : {Counter(y_preds)}")
