...
batch_size = 320  # 训练时每个batch中的样本数 k-folds = 10
patience = 5  # 早停轮数
file_name = 'k_folds_10'  # 指定输出文件的名字
...
for fold, (train_index, valid_index) in enumerate(skf.split(all_label, all_label)):#K-Folds 交叉验证
    ...
    # 得到一折模型对测试集的预测结果
    model.load_state_dict(torch.load('./model_save/' + file_name + '/best_{}.bin'.format(fold)))
